...
// We have hand-written ASM for 4x4, 8x8, 8x16, 16x8, 16x16, 32x32, 64x64,
// and 128x128 blocks, so we can use those for other block sizes as well.
macro_rules! get_sad_neon {
  ($(($W:expr, $H:expr, $BW:expr, $BH:expr)),*) => {
    $(
      paste::item! {
        unsafe extern fn [<rav1e_sad $W x $H _neon>](
          src: *const u8, src_stride: isize, dst: *const u8, dst_stride: isize,
        ) -> u32 {
          let mut sum = 0;
          for w in (0..$W).step_by($BW) {
            for h in (0..$H).step_by($BH) {
              sum += [<rav1e_sad $BW x $BH _neon>](
                src.offset(w + h * src_stride),
                src_stride,
                dst.offset(w + h * dst_stride),
                dst_stride
              );
            }
          }
